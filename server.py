#!/usr/bin/env python3
"""
Nager MCP v101 MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nager-mcp-v101_mcp')
...
logger.info("="*80)
logger.info(f"Nager MCP v101 MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager MCP v101 MCP Server", host="0.0.0.0")
...
# - NAGER_MCP_V101_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-mcp-v101-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nager-mcp-v101-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager MCP v101 MCP Server")
    logger.info("="*80)
