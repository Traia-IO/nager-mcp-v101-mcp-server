...
# Script to build and run the Nager MCP v101 MCP Server locally in Docker
...
IMAGE_NAME="nager-mcp-v101-mcp-server"
CONTAINER_NAME="nager-mcp-v101-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager MCP v101 MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager MCP v101 MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager MCP v101 MCP Server is running!${NC}"
echo
